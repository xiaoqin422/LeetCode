 ...
 *
 * @author 秦笑笑
 * @date 2022-04-04 15:43:42
 */
public class BiaoShiShuZhiDeZiFuChuanLcof {
    public static void main(String[] args) {
        Solution solution = new BiaoShiShuZhiDeZiFuChuanLcof().new Solution();
        System.out.println(solution.isNumber("te1"));
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public boolean isNumber(String s) {
            s = s.trim().toLowerCase();//去除空格
            if (s.length() == 0 || s.contains(" ")) return false;
            if (s.contains("e")){
                String[] split = s.split("e");
                if (split.length != 2) return false;
                else {
                    return (isZNumber(split[0]) || isXNumber(split[0])) && isZNumber(split[1]);
                }
            }else {
                return isZNumber(s) || isXNumber(s);
            }
        }

        private boolean isXNumber(String number) {
            if (number.length() == 0) return false;
            if (number.startsWith("+") || number.startsWith("-")){
                number = number.substring(1);
            }
            if (number.contains("+") || number.contains("-")) return false;
            if (number.indexOf(".") != number.lastIndexOf(".")) return false;
            if (number.startsWith(".")){
                return isZNumber(number.substring(1));
            }else {
                String[] split = number.split("\\.");
                if (split.length > 2 || split.length <= 0) return false;
                if (split.length == 1){
                    return isZNumber(split[0]);
                }else {
                    return isZNumber(split[0]) && isZNumber(split[1]);
                }
            }
        }

        private boolean isZNumber(String number) {
            if (number.contains(".") || number.length() == 0) return false;
            if (number.startsWith("+") || number.startsWith("-")){
                number = number.substring(1);
            }
            try {
